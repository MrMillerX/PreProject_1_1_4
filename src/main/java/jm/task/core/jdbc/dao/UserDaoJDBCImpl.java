...
public class UserDaoJDBCImpl implements UserDao {
    ...
    public void createUsersTable() {
        try (Statement state = JDBCUtil.getNewConnection().createStatement()) {
    ...
    public void saveUser(String name, String lastName, byte age) {
        try (PreparedStatement state = JDBCUtil.getNewConnection()
    ...
    public void removeUserById(long id) {
        try (PreparedStatement state = JDBCUtil.getNewConnection().prepareStatement("DELETE FROM users WHERE id = ?")) {
            state.setLong(1, id);
            state.execute();
        } catch (SQLException ex) {
            ex.printStackTrace();
